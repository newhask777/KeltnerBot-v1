...
TIME_WINDOW = 5 * 60                            # 15 минут в секундах
...
class OIMonitor:
    ...
    def __init__(self, symbols, tg_token, tg_chat_id, instance_id, api_key, api_secret):
        ...
        self.daily_counts = defaultdict(int)    # symbol -> кол-во сигналов за сегодня
        self.loop = None
        self.ws = None
    ...
    async def process_ticker(self, message):
        ...
        if result:
            ...
            if ts - last_alert > COOLDOWN_SECONDS:
                ...
                logger.info(f"[{self.instance_id}] Рост OI {symbol}: {change_percent:.2f}% (сигнал #{signal_number})")
