...
class OIMonitor:
    ...
    def handle_ticker(self, message):
        self._run_coro(self.process_ticker(message))

    def handle_trade(self, message):
        self._run_coro(self.process_trade(message))
    ...
    async def run(self):
        ...
        while self.running:
            try:
                ...
                self.ws.trade_stream(symbol=self.symbols, callback=self.handle_trade)

                logger.info(f"[{self.instance_id}] Запущен, ожидание...")

                # Ждём, пока соединение не будет потеряно (цикл с проверкой состояния)
                while self.running and self.ws and self.ws.is_connected():
                    await asyncio.sleep(1)

                if self.running:
                    logger.warning(f"[{self.instance_id}] Соединение потеряно. Переподключение через 5 секунд...")
            except Exception as e:
                logger.error(f"[{self.instance_id}] Ошибка в цикле: {e}")
            finally:
                self.stop()
