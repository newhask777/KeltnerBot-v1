...
QTY = 150  # размер ордера в BTC (измените под свой риск)
    ...
    while True:
        try:
            ...
            if not in_position:
                ...
                # 5. Логика выхода (смена тренда по Supertrend)
                if latest['supertrend_direction'] == -1 and position['side'] == "Buy":
                    print(f"🔴 Выход из LONG: тренд сменился на медвежий")
                    close_position()
                elif latest['supertrend_direction'] == 1 and position['side'] == "Sell":
                    print(f"🟢 Выход из SHORT: тренд сменился на бычий")
                    close_position()
            
            # 6. Пауза до следующей свечи (~55 минут)
            print(f"⏳ Ожидание 1 минут...")
            time.sleep(60)
