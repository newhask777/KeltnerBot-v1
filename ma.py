...
def place_order_v5(signal):
    try:
        # Закрываем противоположную позицию перед открытием
        if signal == 'long' and get_current_position() == 'sell':
            close_position()
        elif signal == 'short' and get_current_position() == 'buy':
            close_position()
