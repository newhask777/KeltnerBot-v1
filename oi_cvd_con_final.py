...
# Глобальные блокировки для файлов, общих для всех экземпляров OIMonitorWebsocket
...
class OIMonitorWebsocket:
    def __init__(self, symbols: List[str], tg_token: str, tg_chat_id: str, instance_id: str,
        ...
        self.feature_buffer = []
        self.buffer_size = 10
        # Локальная блокировка для буфера (не нужна для файла, т.к. используется глобальная)
    ...
    # ------------------ Работа с CSV (с глобальными блокировками) ------------------
    async def _save_features_to_csv(self):
        """Сохраняет накопленные признаки в общий файл signals.csv"""
    ...
    async def process_ticker(self, message: dict):
        ...
        # Сохраняем все цены в общий файл с глобальной блокировкой
        ...
        # --- СОХРАНЯЕМ ЦЕНУ СИГНАЛА (с глобальной блокировкой) ---
