...
DATA_DIR = "data"
PRICES_DB = os.path.join(DATA_DIR, "prices.db")   # SQLite база для всех цен
...
DB_BATCH_SIZE = 50          # количество записей для пакетной вставки
DB_RETENTION_DAYS = 7         # хранить цены только за последние 7 дней
...
# Глобальные блокировки для CSV файлов (не для БД)
_signal_prices_lock = asyncio.Lock()
_signals_lock = asyncio.Lock()


class PriceDatabase:
    """Асинхронный менеджер для SQLite с буферизацией вставок и автоматической очисткой"""
    ...
    async def init(self):
        """Создаёт таблицу и индексы, запускает фоновую очистку"""
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute("CREATE INDEX IF NOT EXISTS idx_symbol_time ON prices(symbol, timestamp)")
            await db.commit()
    ...
    async def add_price(self, ts: float, symbol: str, price: float):
        """Буферизирует запись цены. Периодически делает массовую вставку."""
        if not self._initialized:
            await self.init()
        async with self.buffer_lock:
            self.buffer.append((ts, symbol, price))
            if len(self.buffer) >= self.batch_size:
                await self._flush()

    async def _flush(self):
        """Вставляет все накопленные записи в БД одной транзакцией"""
        async with self.buffer_lock:
            if not self.buffer:
                return
            to_insert = self.buffer.copy()
            self.buffer.clear()
        async with aiosqlite.connect(self.db_path) as db:
    ...
    async def _cleanup_loop(self):
        """Фоновый цикл: раз в сутки удаляет записи старше DB_RETENTION_DAYS"""
        while True:
            try:
                await asyncio.sleep(86400)  # 24 часа
                cutoff = (datetime.now() - timedelta(days=DB_RETENTION_DAYS)).timestamp()
                async with aiosqlite.connect(self.db_path) as db:
                    await db.execute("DELETE FROM prices WHERE timestamp < ?", (cutoff,))
                    await db.commit()
                    logger.info(f"Очистка БД: удалены записи старше {DB_RETENTION_DAYS} дней")
            except asyncio.CancelledError:
                break
            except Exception as e:
                logger.error(f"Ошибка при очистке БД: {e}")

    async def get_price_after(self, symbol: str, after_ts: float) -> Optional[float]:
        """Возвращает первую цену для символа с timestamp >= after_ts (для разметки)"""
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute(
                "SELECT price FROM prices WHERE symbol = ? AND timestamp >= ? ORDER BY timestamp LIMIT 1",
                (symbol, after_ts)
            ) as cursor:
    ...
    async def flush_and_close(self):
        """Принудительно сохраняет буфер и закрывает фоновую задачу"""
        await self._flush()
...
# Глобальный экземпляр базы данных (будет создан при старте)
price_db = PriceDatabase(PRICES_DB)
...
class OIMonitorWebsocket:
    ...
    # ------------------ Работа с CSV признаков (без изменений) ------------------
    async def _save_features_to_csv(self):
        ...
        async with self.buffer_lock:
            if not self.feature_buffer:
                return
            to_write = self.feature_buffer.copy()
    ...
    async def process_ticker(self, message: dict):
        if not hasattr(self, '_logged_first'):
            logger.info(f"[{self.instance_id}] Первое ticker сообщение: {list(message.keys())}")
        ...
        # Сохраняем цену в SQLite (буферизированно)
        await price_db.add_price(ts, symbol, price)
        ...
        # --- СОХРАНЯЕМ ЦЕНУ СИГНАЛА В CSV (для совместимости с существующими скриптами) ---
        async with _signal_prices_lock:
...
async def main():
    # Инициализируем базу данных перед запуском мониторов
    await price_db.init()
    ...
    try:
        ...
        for m in monitors:
            m.stop()
        # Сохраняем все оставшиеся цены в БД и закрываем её
        await price_db.flush_and_close()
        logger.info("Все мониторы остановлены, буферы сохранены, БД закрыта.")
