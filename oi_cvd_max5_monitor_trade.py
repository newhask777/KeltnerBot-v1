...
BYBIT_API_KEY = os.getenv("BYBIT_API_KEY", "qPbCzwc7jJdzc2hRFS")
BYBIT_API_SECRET = os.getenv("BYBIT_API_SECRET", "bOHhhBbFArHGj6xsf5Ma2nRsf1ezpox03t2k")
...
# ---------- КОМБИНИРОВАННЫЙ ФИЛЬТР РОСТА ЦЕНЫ ЗА 24 ЧАСА ----------
MIN_PRICE_GAIN_24H = 0.1          # минимальный рост цены за 24ч в % (2%)
MAX_PRICE_GAIN_24H = 10.0         # максимальный рост цены за 24ч в % (20%)

# ------------------ ТОРГОВЫЕ НАСТРОЙКИ ------------------
TRADING_ENABLED = True                    # включить автоматическую торговлю
RISK_PERCENT = 10                         # процент от свободного баланса на сделку (2%)
TAKE_PROFIT_PERCENT = 5.0                 # тейк-профит в % от цены входа
STOP_LOSS_PERCENT = 10.0                  # стоп-лосс в % от цены входа
...
class OIMonitor:
    ...
    async def open_long(self, symbol: str, current_price: float) -> bool:
        ...
        try:
            ...
            # Пытаемся установить плечо, но игнорируем ошибки
            if USE_LEVERAGE:
                try:
    ...
    async def process_ticker(self, message):
        ...
        # Комбинированный фильтр роста цены за 24ч (минимальный и максимальный)
        if not (MIN_PRICE_GAIN_24H <= price_24h_change_pct <= MAX_PRICE_GAIN_24H):
            logger.debug(f"[{self.instance_id}] {symbol} отклонён: рост за 24ч = {price_24h_change_pct:.2f}% "
                         f"(диапазон {MIN_PRICE_GAIN_24H}%-{MAX_PRICE_GAIN_24H}%)")
            return
        ...
        msg = (f"🚀 <b>РОСТ OI И CVD (диапазон {MIN_PRICE_GAIN_24H}%-{MAX_PRICE_GAIN_24H}%)</b>\n"
               f"Монета: {symbol}\n"
