...
OI_THRESHOLD = 1.5                             # порог роста OI за 15 минут (в процентах)
...
class OIMonitor:
    ...
    async def _save_features_to_csv(self):
        ...
        async with aiofiles.open(SIGNALS_CSV, mode='a', newline='', encoding='utf-8') as f:
            if not file_exists:
                # Заголовки: timestamp, symbol, oi_change_5m, oi_change_15m, oi_change_1h,
                # price_change_5m, price_change_15m, price_change_1h, volume, volatility
                await f.write("timestamp,symbol,oi_change_5m,oi_change_15m,oi_change_1h,"
                              "price_change_5m,price_change_15m,price_change_1h,volume,volatility\n")
    ...
    def _get_value_at(self, history_dict, symbol, target_ts, default=None):
        ...
        # Ближайшее значение с timestamp <= target_ts
    ...
    def _get_change(self, symbol, history_dict, current_ts, current_val, window_sec):
        """
        Возвращает процентное изменение значения за window_sec секунд до current_ts.
        """
    ...
    def _collect_features(self, symbol, current_ts, oi_now, price_now):
        """
        Собирает признаки для текущего момента (без привязки к сработавшему порогу).
        Возвращает список:
        [timestamp, symbol,
         oi_change_5m, oi_change_15m, oi_change_1h,
         price_change_5m, price_change_15m, price_change_1h,
         volume, volatility_15m]
        """
        # Изменения OI
        oi_change_5m = self._get_change(symbol, self.oi_history, current_ts, oi_now, 300)
        oi_change_15m = self._get_change(symbol, self.oi_history, current_ts, oi_now, 900)
        oi_change_1h = self._get_change(symbol, self.oi_history, current_ts, oi_now, 3600)

        # Изменения цены
        price_change_5m = self._get_change(symbol, self.price_history, current_ts, price_now, 300)
        price_change_15m = self._get_change(symbol, self.price_history, current_ts, price_now, 900)
        price_change_1h = self._get_change(symbol, self.price_history, current_ts, price_now, 3600)

        # Объём (последний известный)
        volume = self._get_latest(self.volume_history, symbol) or 0.0

        # Волатильность за 15 минут
        volatility = self._calc_volatility(symbol, current_ts, 900)
    ...
    def _is_good_pattern(self, features):
        """
        features – полный список из 10 элементов (timestamp, symbol, ... числовые признаки).
        Проверяем, относится ли сигнал к хорошему кластеру.
        """
        if self.scaler is None or self.kmeans is None or self.good_clusters is None:
            return True
        try:
            # Извлекаем 8 числовых признаков: oi_change_5m, oi_change_15m, oi_change_1h,
            # price_change_5m, price_change_15m, price_change_1h, volume, volatility
            X = np.array(features[2:10]).reshape(1, -1)
    ...
    async def process_ticker(self, message):
        ...
        # Проверяем рост OI за 15 минут (основной триггер)
        hist = self.oi_history.get(symbol)
        if not hist or len(hist) < 2:
            return

        # Ищем значение 15 минут назад (900 секунд)
        target_ts = ts - 900
    ...
    async def run(self):
        ...
        try:
            self.ws.ticker_stream(symbol=self.symbols, callback=self.handle_ticker)
        except Exception as e:
