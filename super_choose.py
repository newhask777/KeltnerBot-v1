...
# ========== НАСТРОЙКИ API BYBIT ==========
API_KEY = os.getenv("BYBIT_API_KEY", "ILGPyknN8m0vNmEgIx")          
API_SECRET = os.getenv("BYBIT_API_SECRET", "eHz4iBZSe0qMXv6gmsxoqudzCqED0mB8apMC")    
...
# Вариант 1: задайте список вручную (если не пуст, файл watchlist.txt игнорируется)
WATCHLIST = ["DOGEUSDT", "HUMAUSDT", "MNTUSDT"]  # например: ["BTCUSDT", "ETHUSDT", "SOLUSDT"]

# Вариант 2: укажите путь к файлу со списком (по умолчанию watchlist.txt)
...
# ========== ФУНКЦИЯ ЗАГРУЗКИ СПИСКА ОТСЛЕЖИВАЕМЫХ ПАР ==========
def load_watchlist():
    """Возвращает список символов для отслеживания"""
    # Если WATCHLIST задан в коде – используем его
    if WATCHLIST:
        return WATCHLIST

    # Иначе пробуем загрузить из файла
    if os.path.exists(WATCHLIST_FILE):
        try:
            with open(WATCHLIST_FILE, 'r') as f:
                symbols = [line.strip().upper() for line in f if line.strip()]
            if symbols:
                logger.info(f"Загружено {len(symbols)} пар из файла {WATCHLIST_FILE}")
                return symbols
            else:
                logger.warning(f"Файл {WATCHLIST_FILE} пуст, будут отслеживаться все пары")
        except Exception as e:
            logger.error(f"Ошибка чтения файла {WATCHLIST_FILE}: {e}")

    # Если ничего не задано – отслеживаем все пары
    logger.info("Список отслеживаемых пар не задан, будут отслеживаться все пары")
    return None
        ...
        try:
            ...
            if "topic" in data and "kline" in data["topic"]:
                ...
                if symbol in indicators:
                    ...
                    logger.debug(f"Получена свеча для неподписанного символа {symbol}")
...
async def main():
    # 1. Загружаем список отслеживаемых пар
    watchlist = load_watchlist()

    # 2. Получаем все доступные линейные пары
    all_symbols = get_all_linear_symbols()
    if not all_symbols:
        logger.error("Не удалось получить список символов")
        return
    logger.info(f"Найдено {len(all_symbols)} линейных символов")

    # 3. Отбираем нужные символы
    if watchlist is not None:
        symbols_to_track = [s for s in watchlist if s in all_symbols]
        not_found = set(watchlist) - set(all_symbols)
        if not_found:
            logger.warning(f"Следующие символы из списка не найдены: {not_found}")
        if not symbols_to_track:
            logger.error("Нет отслеживаемых символов из списка")
            return
        logger.info(f"Будут отслеживаться {len(symbols_to_track)} пар: {', '.join(symbols_to_track[:10])}" +
                   (f" и ещё {len(symbols_to_track)-10}" if len(symbols_to_track) > 10 else ""))
    else:
        symbols_to_track = all_symbols
        logger.info(f"Будут отслеживаться все {len(symbols_to_track)} пар")

    # 4. Инициализируем индикаторы с историей
    indicators = {}
    ...
    # 5. Подключение к WebSocket и подписка
